...
@section('content')
    <div class="container">
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-header py-2"><strong>Assign Subjects to Classes</strong></div>
            <div class="card-body p-3">
                ...
                @if ($selectedSchool)
                    <form method="POST" action="{{ route('class-subjects.store') }}">
                        @csrf
                        <div class="mb-3">
                            ...
                            <select name="class_id" class="form-select form-select-sm" required>
                                ...
                                @foreach ($classes as $class)
                                    <option value="{{ $class->id }}">
                                        {{ $class->name }} | Section: {{ $class->section }} | Session:
                                        {{ $class->session_year }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="card mt-2 mb-2">
                            <div class="card-header py-1 small fw-semibold">
                                ...
                                <span class="text-muted">
                                    | Section: {{ $class->section }} | Session: {{ $class->session_year }}
                                </span>
                            </div>
                            <div class="card-body py-2">
                                @forelse($class->subjects as $subject)
                                    <span class="badge bg-secondary rounded-pill me-1 mb-1">
                                        {{ $subject->name }}
                                        <form method="POST"
                                            action="{{ route('class-subjects.destroy', [$class->id, $subject->id]) }}"
                                            class="d-inline">
                                            @csrf
                                            @method('DELETE')
