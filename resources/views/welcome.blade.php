                ...
                @auth
                    <a href="{{ auth()->check()
                        ? (auth()->user()->roles->contains('name', 'student')
                            ...
                            : url(auth()->user()->firstAccessibleScreen() ?? '/default-dashboard'))
                        : url('/dashboard') }}"
                    ...
                    <form method="POST" action="{{ route('logout') }}" style="display:inline">
                        @csrf
                        <button type="submit"
                            style="background:none; border:none; padding:0; color:#f6f6f0; cursor:pointer; "
                            class="inline-block dark:text-[#EDEDEC] text-sm leading-normal">
                            Logout
                        </button>
                    </form>
                @else
